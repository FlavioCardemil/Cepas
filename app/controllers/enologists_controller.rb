class EnologistsController < ApplicationController
  ...
  def edit
    @magazines = Magazine.all
    @enologist = Enologist.new
    @roles = EnologistMagazine.roles.keys
  end
  ...
  private
    ...
    def enologist_params
      params.require(:enologist).permit(:name, :age, :nacionality, enologist_magazines_attributes: [:id, :magazine_id, :editor, :writer, :reviewer])
    end
end
